...
def check_dist(node,graph):
	...
	# import pdb; pdb.set_trace()
	print(graph[65,300])
	if  node[0]+cl2 <= shape[0]-1 and node[0]-cl2 >= 0 and node[1]+cl2 <= shape[1]-1 and node[1]-cl2 >= 0 and graph[node[0]+cl2,node[1]+cl2] == 0 and graph[node[0]-cl2,node[1]+cl2] == 0 and graph[node[0]-cl2,node[1]-cl2] == 0  and graph[node[0]+cl2,node[1]-cl2]:
		return True
	return False

def move_up(node, graph):
	# import pdb; pdb.set_trace()
	if xor(check_dist(node,graph), cb(node,graph)):
